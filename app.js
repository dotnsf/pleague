...
app.get( '/svg', function( req, res ){
  ...
  var pins3 = req.query.pins3;
  if( pins1 ){
    res.contentType( 'image/svg+xml' );
    var svg = toSVG( pins1, pins2, pins3 );
    //console.log( svg );
    res.write( svg );
    res.end();
  }else{
...
app.get( '/stats', function( req, res ){
  ...
  connection.query( sql1, params1, function( error1, games, fields1 ){
    if( error1 ){
      console.log( 'error1' );
      console.log( error1 );
      res.write( JSON.stringify( { status: false, sql: sql1, params: params1, error: error1 }, 2, null ) );
      res.end();
    }else{
      ...
      games.forEach( function( game ){
        ...
        connection.query( sql2, params2, function( error2, game_scores, fields2 ){
          if( error2 ){
            console.log( 'error2' );
            console.log( error2 );
            res.write( JSON.stringify( { status: false, sql: sql2, params: params2, error: error2 }, 2, null ) );
            res.end();
          }else{
            //console.log( '#game_scores: ' + game_scores.length );
            var game_scores_length = game_scores.length;
            var game_scores_count = 0;

            //. #game_scores == 1 とは限らない（3かも）
            game_scores.forEach( function( game_score ){
              var score = game_score.score;
              var start_lane = game_score.start_lane;
              ...
              if( member_id ){
                ...
                params3.push( game_score.member_id );
                sql3 += ' and member_id = ?'
              }
              ...
              connection.query( sql3, params3, function( error3, flame_scores, fields3 ){
                if( error3 ){
                  console.log( 'error3' );
                  console.log( error3 );
                  res.write( JSON.stringify( { status: false, sql: sql3, params: params3, error: error3 }, 2, null ) );
                  res.end();
                }else{
                  //. flame_scores が準備できていないケースだと、ここは 0 になり、Response を返す処理に行かない
                  //console.log( '#flame_scores: ' + flame_scores.length );
                  var flame_scores_length = flame_scores.length;

                  if( flame_scores.length > 0 ){
                    ...
                    flame_scores.forEach( function( flame_score ){
                      ...
                      flame_scores_count ++;
                      //console.log( ' flame_scores: ' + flame_scores_count + '/' + flame_scores_length );
                  ...
                  //console.log( ' game_scores: ' + game_scores_count + '/' + game_scores_length );

                  if( game_scores_count == game_scores_length ){
                    games_count ++;
                    //console.log( ' games: ' + games_count + '/' + games_length );
...
function toSVG( v1, v2, v3 ){
  var arr1 = null, arr2 = null, arr3 = null;
  if( Array.isArray( v1 ) ){
    arr1 = v1;
  ...
  var svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + width + ' 80">';
...
*/
    ...
    if( Array.isArray( arr2 ) && arr2.length == 10 ){
      if( v1 == 1023 ){
        svg += '<g font-size="20">';
        svg += '<text x="110" y="78">' + ( arr2[0] == 1 ? '○' : '①' ) + '</text>';
        svg += '<text x="100" y="58">' + ( arr2[1] == 1 ? '○' : '②' ) + '</text>';
        svg += '<text x="120" y="58">' + ( arr2[2] == 1 ? '○' : '③' ) + '</text>';
        svg += '<text x="90" y="38">' + ( arr2[3] == 1 ? '○' : '④' ) + '</text>';
        svg += '<text x="110" y="38">' + ( arr2[4] == 1 ? '○' : '⑤' ) + '</text>';
        svg += '<text x="130" y="38">' + ( arr2[5] == 1 ? '○' : '⑥' ) + '</text>';
        svg += '<text x="80" y="18">' + ( arr2[6] == 1 ? '○' : '⑦' ) + '</text>';
        svg += '<text x="100" y="18">' + ( arr2[7] == 1 ? '○' : '⑧' ) + '</text>';
        svg += '<text x="120" y="18">' + ( arr2[8] == 1 ? '○' : '⑨' ) + '</text>';
        svg += '<text x="140" y="18">' + ( arr2[9] == 1 ? '○' : '⑩' ) + '</text>';
        svg += '</g>';

        if( Array.isArray( arr3 ) && arr3.length == 10 ){
          if( v2 == 1023 ){
            svg += '<g font-size="20">';
            svg += '<text x="190" y="78">' + ( arr3[0] == 1 ? '○' : '①' ) + '</text>';
            svg += '<text x="180" y="58">' + ( arr3[1] == 1 ? '○' : '②' ) + '</text>';
            svg += '<text x="200" y="58">' + ( arr3[2] == 1 ? '○' : '③' ) + '</text>';
            svg += '<text x="170" y="38">' + ( arr3[3] == 1 ? '○' : '④' ) + '</text>';
            svg += '<text x="190" y="38">' + ( arr3[4] == 1 ? '○' : '⑤' ) + '</text>';
            svg += '<text x="210" y="38">' + ( arr3[5] == 1 ? '○' : '⑥' ) + '</text>';
            svg += '<text x="160" y="18">' + ( arr3[6] == 1 ? '○' : '⑦' ) + '</text>';
            svg += '<text x="180" y="18">' + ( arr3[7] == 1 ? '○' : '⑧' ) + '</text>';
            svg += '<text x="200" y="18">' + ( arr3[8] == 1 ? '○' : '⑨' ) + '</text>';
            svg += '<text x="220" y="18">' + ( arr3[9] == 1 ? '○' : '⑩' ) + '</text>';
            svg += '</g>';
          }else{
            if( arr3[0] == 1 ){ svg += '<line x1="110" y1="80" x2="130" y2="60" stroke="#000000"/>'; }
            if( arr3[1] == 1 ){ svg += '<line x1="100" y1="60" x2="120" y2="40" stroke="#000000"/>'; }
            if( arr3[2] == 1 ){ svg += '<line x1="120" y1="60" x2="140" y2="40" stroke="#000000"/>'; }
            if( arr3[3] == 1 ){ svg += '<line x1="90" y1="40" x2="110" y2="20" stroke="#000000"/>'; }
            if( arr3[4] == 1 ){ svg += '<line x1="110" y1="40" x2="130" y2="20" stroke="#000000"/>'; }
            if( arr3[5] == 1 ){ svg += '<line x1="130" y1="40" x2="150" y2="20" stroke="#000000"/>'; }
            if( arr3[6] == 1 ){ svg += '<line x1="80" y1="20" x2="100" y2="0" stroke="#000000"/>'; }
            if( arr3[7] == 1 ){ svg += '<line x1="100" y1="20" x2="120" y2="0" stroke="#000000"/>'; }
            if( arr3[8] == 1 ){ svg += '<line x1="120" y1="20" x2="140" y2="0" stroke="#000000"/>'; }
            if( arr3[9] == 1 ){ svg += '<line x1="140" y1="20" x2="160" y2="0" stroke="#000000"/>'; }
          }
        }
      }else{
        ...
        if( Array.isArray( arr3 ) && arr3.length == 10 ){
          if( v1 + v2 == 1023 ){
            svg += '<g font-size="20">';
            svg += '<text x="110" y="78">' + ( arr3[0] == 1 ? '○' : '①' ) + '</text>';
            svg += '<text x="100" y="58">' + ( arr3[1] == 1 ? '○' : '②' ) + '</text>';
            svg += '<text x="120" y="58">' + ( arr3[2] == 1 ? '○' : '③' ) + '</text>';
            svg += '<text x="90" y="38">' + ( arr3[3] == 1 ? '○' : '④' ) + '</text>';
            svg += '<text x="110" y="38">' + ( arr3[4] == 1 ? '○' : '⑤' ) + '</text>';
            svg += '<text x="130" y="38">' + ( arr3[5] == 1 ? '○' : '⑥' ) + '</text>';
            svg += '<text x="80" y="18">' + ( arr3[6] == 1 ? '○' : '⑦' ) + '</text>';
            svg += '<text x="100" y="18">' + ( arr3[7] == 1 ? '○' : '⑧' ) + '</text>';
            svg += '<text x="120" y="18">' + ( arr3[8] == 1 ? '○' : '⑨' ) + '</text>';
            svg += '<text x="140" y="18">' + ( arr3[9] == 1 ? '○' : '⑩' ) + '</text>';
            svg += '</g>';
          }else{
          }
        }
